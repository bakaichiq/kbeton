...
Excel используется **только как входной формат выгрузки из 1С (XLSX)**.

## Что реализовано

- Telegram Bot (**aiogram 3**) с меню:
  - 📊 Дашборд
  - 💰 Финансы: P&L, статьи, правила маппинга, неразобранное (ручная разметка), импорт 1С, импорт взаиморасчетов, цены (FinDir/Admin)
...
- Импорт XLSX (финансы, взаиморасчеты) через S3/MinIO + Celery worker
- Дедупликация строк импорта по `sha256` ключевых полей
...
2) Admin/FinDir:
   - 💰 Финансы → 📥 Загрузить выгрузку 1С (финансы)
...
### Финансы (выгрузка 1С)
Команда бота: **📥 Загрузить выгрузку 1С (финансы)**  
Файл сохраняется в MinIO (`imports/finance/...`), затем Celery обрабатывает и пишет `finance_transactions`.

Поддерживаются синонимы заголовков. Пример шаблона:
- `samples/finance_1c_template.xlsx`
